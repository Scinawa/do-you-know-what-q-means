...
int main (void) {
	...
	default_random_engine generator;
    	...
    	double time_standard_Kmeans[n_parallel][length_N] = {0};
    	double time_EK_Kmeans[n_parallel][length_N][length_eps] = {0};
    	
    	double iterations_standard_Kmeans[n_parallel][length_N] = {0};
    	double iterations_EK_Kmeans[n_parallel][length_N][length_eps] = {0};
    	   	
    	double RSS[n_parallel][length_N][length_eps] = {0};
    	double ARI[n_parallel][length_N][length_eps] = {0};
    	double NMI[n_parallel][length_N][length_eps] = {0};
	 ...
	 //////////////////////////////////////////////////////////////////////////////////////
					...
					iterations_standard_Kmeans[parallel][n] += iterations / (n_iter_C * n_iter_N);

				    	// Compute duration
				    	auto end = chrono::high_resolution_clock::now();
				    	double dur = chrono::duration_cast<chrono::milliseconds>(end - begin).count();
				    	time_standard_Kmeans[parallel][n] += dur / (n_iter_N * n_iter_C);
...
//						while (distance_centroids > K * threshold) {
						...
						iterations_EK_Kmeans[parallel][n][e] += iterations / (n_iter_C * n_iter_N);

					    	// Compute duration
					    	end = chrono::high_resolution_clock::now();
					    	dur = chrono::duration_cast<chrono::milliseconds>(end - begin).count();
					    	time_EK_Kmeans[parallel][n][e] += dur / (n_iter_N * n_iter_C);
					...
					//    	cout << "Iterations: " << iterations << endl;
					    	...
					    	RSS[parallel][n][e] += 100 * (RSS_eps - RSS_standard) / RSS_standard / (n_iter_C * n_iter_N);
					 //   	cout << "RSS: " << RSS[parallel][n][e] << endl;				    	
					    	...
					    	ARI[parallel][n][e] += (Nij - 2 * ai * bj / N[n] / (N[n]-1) ) / (ai/2 + bj/2 - 2 * ai * bj / N[n] / (N[n]-1) ) / (n_iter_C * n_iter_N);
					    	NMI[parallel][n][e] += 2 * MI / (H_standard + H_eps) / (n_iter_C * n_iter_N);
					 //   	cout << "ARI: " << ARI[parallel][n][e] << endl;
					 //   	cout << "NMI: " << NMI[parallel][n][e] << endl;
					}
	...
	MyFile << "iterations" << ' ' << n_parallel * n_iter_C * n_iter_N << endl;
	
	for (int n = 0; n < length_N; n++) {
		double time = 0;
		double iterations = 0;
		for (int parallel = 0; parallel < n_parallel; parallel++) {
			time += time_standard_Kmeans[parallel][n] / n_parallel;
			iterations += iterations_standard_Kmeans[parallel][n] / n_parallel;
		}
		MyFile << N[n] << ' ' << 0.0 << ' ' << time << ' ' << iterations << endl;
		cout << "N: " << N[n] << endl;
		...
		for (int e = 0; e < length_eps; e++) {
			...
			iterations = 0;
	    		...
	    		for (int parallel = 0; parallel < n_parallel; parallel++) {
				time += time_EK_Kmeans[parallel][n][e] / n_parallel;
				iterations += iterations_EK_Kmeans[parallel][n][e] / n_parallel;
				RSS_aux += RSS[parallel][n][e] / n_parallel;
				ARI_aux += ARI[parallel][n][e] / n_parallel;
				NMI_aux += NMI[parallel][n][e] / n_parallel;
			}
			MyFile << N[n] << ' ' << eps[e] << ' ' << time << ' ' << iterations << ' ' << RSS_aux << ' ' << ARI_aux << ' ' << NMI_aux << endl;
